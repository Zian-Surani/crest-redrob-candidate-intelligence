      ...
      <DataState loading={loading} error={error}>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-premium shadow-premium border border-slate-200/60 overflow-hidden">
          <div className="p-4 border-b border-slate-100 flex items-center space-x-4 bg-slate-50/50">
            <div className="flex-1 relative"><span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-[18px]">search</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search name, role, company, evidenceâ€¦" className="w-full bg-white border border-slate-200 rounded-xl pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" /></div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100">{visible.map((candidate) => (
                <tr key={candidate.candidate_id} className="hover:bg-slate-50/80 transition-colors group">
                  ...
                  <td className="px-5 py-4"><div className="font-medium text-slate-800 text-sm">{candidate.role} Â· {candidate.company}</div><div className="text-xs text-slate-500 mt-0.5"><span className="material-symbols-outlined text-[13px] align-middle mr-1">location_on</span>{candidate.location} Â· {candidate.years_experience} yrs</div></td>
                  ...
                  <td className="px-5 py-4"><p className={`text-sm font-bold ${candidate.projected_cph_inr <= 110000 ? 'text-success' : 'text-warning'}`}>{inr(candidate.projected_cph_inr, true)}</p><p className="text-[10px] text-slate-400">benchmark â‚¹1.1L</p></td>
      ...
      {runOpen && (
        <div className="fixed inset-0 z-[110] bg-slate-900/30 backdrop-blur-sm flex items-center justify-center p-6">
          <form onSubmit={runRanking} className="bg-white rounded-premium shadow-2xl border border-slate-200 w-full max-w-md p-7">
            ...
            <label className="block text-xs font-bold text-slate-600 mb-2">Job description</label><select name="job_id" required className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-3 text-sm mb-4">{(jobs || []).map((job) => <option key={job.id} value={job.id}>{job.title} Â· {job.company}</option>)}</select>
