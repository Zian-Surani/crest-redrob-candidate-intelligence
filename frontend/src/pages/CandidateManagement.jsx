  ...
  const runRanking = async (event) => {
    ...
    try {
      await api('/rankings/run', {
        method: 'POST',
        body: JSON.stringify({ job_id: Number(form.get('job_id')), scope: form.get('scope'), limit: form.get('scope') === 'sample' ? 50 : 100 }),
      })
      setRunOpen(false)
      await refresh()
      ...
      {runOpen && (
        <div className="fixed inset-0 z-[110] bg-slate-900/30 backdrop-blur-sm flex items-center justify-center p-6">
          <form onSubmit={runRanking} className="bg-white rounded-premium shadow-2xl border border-slate-200 w-full max-w-md p-7">
            <div className="flex justify-between mb-6"><div><h2 className="text-xl font-bold text-slate-900">Run candidate ranking</h2><p className="text-sm text-slate-500 mt-1">The full run streams all 100,000 official profiles.</p></div><button type="button" onClick={() => setRunOpen(false)}><span className="material-symbols-outlined text-slate-400">close</span></button></div>
            ...
            <label className="block text-xs font-bold text-slate-600 mb-2">Dataset scope</label><select name="scope" defaultValue="full" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-3 text-sm mb-5"><option value="full" disabled={!dataset?.full_available}>Official 100K pool {dataset?.full_available ? '(submission)' : '(not mounted in sandbox)'}</option><option value="sample">Official 50-candidate sample (diagnostic only)</option></select>
            <button disabled={running || !jobs?.length} className="w-full bg-primary text-white rounded-xl py-3 text-sm font-bold disabled:opacity-50">{running ? 'Ranking candidates…' : 'Run evidence pipeline'}</button>
          </form>
        </div>
      )}
    </div>
