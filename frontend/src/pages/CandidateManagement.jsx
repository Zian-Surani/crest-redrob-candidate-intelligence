      ...
      {runOpen && (
        <div className="fixed inset-0 z-[110] bg-slate-900/30 backdrop-blur-sm flex items-center justify-center p-6">
          <form onSubmit={runRanking} className="bg-white rounded-premium shadow-2xl border border-slate-200 w-full max-w-md p-7">
            ...
            <label className="block text-xs font-bold text-slate-600 mb-2">Dataset scope</label><select name="scope" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-3 text-sm mb-5"><option value="sample">50-candidate demo (instant)</option><option value="full" disabled={!dataset?.full_available}>Official 100K pool {dataset?.full_available ? '(submission)' : '(not mounted in sandbox)'}</option></select>
